...
- [x] **T01: Closed-enum schema extensions + atomic id/asset/test migration** `est:3h`
...
- [x] **T02: GameplayCommandForbidden validation check + anti-DRY test + EmitDamage remediation** `est:2h30m`
...
- [x] **T03: SkillGraphRegistry + StanceGraphRegistry (pure id->Handle resolution, R008)** `est:2h`
...
- [x] **T04: Agumon Stance FSM asset + whole-sheet clip range + stance load path** `est:2h`
...
- [x] **T05: Runtime AnimGraph player + RenderPlugin/UiPlugin split (windowed Agumon idle)** `est:5h`
