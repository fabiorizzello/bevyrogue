...
| Headless curve-eval contracts (`cargo test --test animation vfx_asset_eval`) | runtime | PASS | 12 passed, 0 failed; key test `sharp_claws_slash_curves_evaluate_deterministically` green |
...
| Windowed render-path contracts (`cargo test --features windowed --test windowed_only vfx_windowed_contracts`) | runtime | PASS | 1 passed, 0 failed; `setup_camera_configures_hdr_bloom_tonemapping_and_deband_dither` green |
...
- D037 deferral (strict additive particle blending) confirmed present in acceptance artifact; Automated evidence stays at technical render-path configuration within Bevy 0.18 2D constraints; visual intent remains human-only.
