...
| D026 | M002 S01 / test-refactor 260520-1 | blueprint/observability | Blueprint `last_transition` snapshot fields | Treat `BatteryLoopSnapshot::last_transition` (tentomon, `src/combat/blueprints/tentomon/identity.rs:260`) and `PredatorLoopSnapshot::last_transition` (dorumon, `src/combat/blueprints/dorumon/identity.rs:263`) as **typed observability contract surfaces**, not internal latches. Mirrored into validation UI (`tentomon/mod.rs:107`, `dorumon/mod.rs:72`) and JSONL log formatting (`tentomon/apply.rs:185-202`). Tests that assert on `last_transition` are the guard against display/contract regressions and MUST NOT be rewritten as "implementation-detail" coupling. | The W8 V4 fragility-rewrite was scoped to remove tests that probe internal latches. On inspection, `last_transition` is consumed by two non-test surfaces (the validation extension registry rows and the per-step JSONL transition log) — deleting those assertions would silently delete the only regression guard those surfaces have. | Yes | agent |
