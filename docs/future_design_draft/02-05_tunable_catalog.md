...
| 10 | **`assets/digimon/<name>/clipmontage.ron`** | ❌ nuovo (§2.2 → forma finale §2.2b) | **AnimGraph FSM** (nodi + edges + Commands `on_enter`); fallback degenerate = 1 nodo all-clip | **alta** | **introdurre** (6 file, solo Agumon popolato in M017 col grafo Baby Flame in `agumon/02_skill_baby_flame.md`; full-featured variant con skill-tree unlocks è deferred post-M017; gli altri 5 = degenerate node) |
