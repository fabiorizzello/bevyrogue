...
| S05 | `status_effects.ron` minimo — solo i 5 status v0 in §8.3 (`Heated`, `Chilled`, `Slowed`, `Confused`, `Paralyzed`), validazione stack rules. Niente status set extension oltre questi. | low | S03 |
...
- **3 spender nuovi (`meltdown_strike`, `frost_lance`, `prey_strike`)** — sostituiti dai 3 skill minimal di §8.3 (`pepper_breath`, `bubble_blast`, `draconic_edge`).
