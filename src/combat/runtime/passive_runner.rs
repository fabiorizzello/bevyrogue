...
mod tests {
    ...
    fn loop_timeline_halts_at_circuit_breaker() {
        fn never(_evt: &BeatEvent, _ctx: &crate::combat::runtime::skill_ctx::SkillCtx<'_>) -> bool {
            false
        }

        fn loop_hook(evt: &BeatEvent, ctx: &mut crate::combat::runtime::skill_ctx::SkillCtx<'_>) {
            use std::sync::atomic::{AtomicU32, Ordering};

            static LOOP_CALLS: AtomicU32 = AtomicU32::new(0);
            let count = LOOP_CALLS.fetch_add(1, Ordering::Relaxed) + 1;
            ctx.enqueue(crate::combat::runtime::intent::Intent::SetBlueprintState {
                actor: ctx.caster,
        ...
        let mut cast_id_gen = crate::combat::runtime::intent::CastIdGen::default();
        ...
        let state = world.resource::<crate::combat::runtime::blueprint_state::BlueprintState>();
        assert_eq!(
