...
use super::{
    UnitSnapshot, allied_basic_skill_ids, emit_event, find_unit_entity, find_unit_snapshot,
};

pub(super) fn apply_deal_damage(
    world: &mut World,
    source: UnitId,
    target: UnitId,
    base_damage: i32,
    ...
    let pre_damage_mitigation: Option<i32> = {
        let fns: Vec<_> = world
            .get_resource::<super::super::ExtRegistries>()
...
pub(super) fn apply_break_toughness(
    world: &mut World,
    source: UnitId,
    target: UnitId,
    amount: i32,
...
pub(super) fn apply_damage_modifier(
    world: &mut World,
    target: UnitId,
    layer: ModifierLayer,
    multiplier_pct: i32,
...
pub(super) fn apply_delay_turn(world: &mut World, target: UnitId, amount_pct: u32, cast_id: CastId) {
    if amount_pct == 0 {
        return;
    }
    emit_event(
...
pub(super) fn apply_advance_turn(world: &mut World, target: UnitId, amount_pct: u32, cast_id: CastId) {
    if amount_pct == 0 {
        return;
    }
    emit_event(
...
pub(super) fn apply_add_energy(world: &mut World, target: UnitId, amount: i32, cast_id: CastId) {
    if amount <= 0 {
        return;
    }
...
pub(super) fn apply_revive(world: &mut World, source: UnitId, target: UnitId, pct: i32, cast_id: CastId) {
...
pub(super) fn apply_grant_free_skill(world: &mut World, source: UnitId, count: usize, cast_id: CastId) {
    if count == 0 {
        return;
    }
...
pub(super) fn apply_blueprint_signal(
    world: &mut World,
    source: UnitId,
    owner: &'static str,
    name: &'static str,
...
pub(super) fn apply_set_blueprint_state(
    world: &mut World,
    actor: UnitId,
    key: String,
    value: i64,
