...
use crate::combat::resolution::{
    TargetEntry, TargetableSnapshot, apply_damage_only, compute_hop_damage, select_bounce_hop,
};
...
pub(super) fn run_hop_loop(
    ...
    for hop_k in 0..clamped_hops {
        ...
        let Ok([att_row, mut def_row]) = actors.get_many_mut([attacker_entity, def_entity]) else {
            continue;
        };

        let (_, _att_team_val, att_unit_val, _, _, _, _, _, _, _, att_bag_val, _, _, _) = &att_row;
        let (
            _,
            def_team_val,
