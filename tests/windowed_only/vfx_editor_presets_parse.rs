//! Proves the `vfx_editor` bin's two test presets (`assets/vfx/fire_test` and
//! `water_test`) deserialize into bevy_enoki's real `Particle2dEffect` schema —
//! the same type the editor loads via `asset_server.load::<Particle2dEffect>`.
//!
//! This is the K001-safe guard for "the effect doesn't show": an enoki asset
//! that fails to parse loads as a silent error (no panic, no particles), so the
//! editor would render an empty scene. Pinning the parse here catches that
//! without opening a window. Both presets exercise the `Rval<Vec2>` `direction`
//! form (`Some(((x, y), randomness))`) that the baby_flame assets never used.
//!
...
fn fire_preset_parses_into_enoki_schema() {
    ...
    // Continuous emitter (the editor attaches no OneShot), so it must keep feeding.
    assert!(
