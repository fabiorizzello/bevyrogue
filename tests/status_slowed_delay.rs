...
///
/// Assertions:
///   1. Exactly one DelayTurn{target=defender_id, amount_pct=30} event after first apply.
...
fn count_delay_for(events: &[CombatEvent], target: UnitId, amount_pct: u32) -> usize {
    events
        .iter()
        .filter(|e| {
            matches!(
                &e.kind,
                CombatEventKind::DelayTurn { target: t, amount_pct: a }
...
/// First apply of Slowed emits exactly one DelayTurn{30} and pushes AV by −3000.
/// Second apply (re-apply) emits zero additional DelayTurn events.
#[test]
fn slowed_first_apply_delays_gauge_reapply_does_not() {
    ...
    let ta_count = count_delay_for(&events1, defender_id, 30);
    assert_eq!(
        ta_count, 1,
        "first apply must emit exactly 1 DelayTurn{{amount_pct:30}}; got {ta_count}\nevents: {events1:?}"
    );

    // apply_av_ops_system must have consumed the DelayTurn event and updated AV.
    ...
    // Confirm OnStatusApplied{Slowed} precedes DelayTurn in the event stream.
    ...
    let ta_count2 = count_delay_for(&events2, defender_id, 30);
    assert_eq!(
        ta_count2, 0,
        "re-apply must emit zero DelayTurn events (refresh_max_dur only); got {ta_count2}\nevents: {events2:?}"
    );
}
