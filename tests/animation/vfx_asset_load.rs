...
use bevyrogue::animation::{
    ImpactSpawnPlan, PlacementAnchor, VfxAsset, VfxValidationError, eval_color, eval_scale,
    resolve_effect, spawn_plan, validate_effects,
};
...
fn agumon_charge_curves_grow_and_brighten() {
    ...
    assert_eq!(
        eval_color(&charge.appearance.color, 0.0),
        [1.5, 1.3, 1.0, 0.4]
    );
    assert_eq!(
        eval_color(&charge.appearance.color, 1.0),
        [1.9, 1.6, 1.2, 0.95]
    );
}
...
fn baby_burner_detonate_is_fan_out_burst_chaining_flash() {
    ...
    let chained = det
        .on_expire
        .as_ref()
        .expect("detonate must chain on_expire");
    assert_eq!(
        chained.0, "baby_burner.flash",
        "detonate chains baby_burner.flash"
    );
}
...
fn baby_burner_flash_curves_match_authored_values() {
    ...
    assert_eq!(
        eval_color(&flash.appearance.color, 0.0),
        [3.6, 2.2, 0.8, 0.95]
    );
    assert_eq!(
        eval_color(&flash.appearance.color, 1.0),
        [3.6, 2.2, 0.8, 0.0]
    );
    assert_rgba_approx(
...
fn agumon_vfx_contains_sharp_claws_slash() {
    ...
    assert!(
        plan.ttl_ticks > 0 && plan.ttl_ticks <= 12,
        "TTL is bounded and short"
    );
    ...
    assert!(
        slash.on_expire.is_none(),
        "Sharp Claws slash is a terminal effect"
    );
}
